...
# Caminho relativo à raiz do projeto (../../ vai para raiz, depois data/raw/IBGE/)
...
def fetch_municipios(estado: int) -> list[dict]:
    ...
    for item in raw:
        ...
        try:
            ...
            continue  # pula o município problemático
...
def write_csv(municipios: list[dict], output_path: str) -> None:
    os.makedirs(os.path.dirname(output_path), exist_ok=True)  # ← adicionar isso
...
if __name__ == '__main__':
    ...
    # 1. Busca municípios + coordenadas
    municipios = fetch_municipios(ESTADO_PR)

    # 2. Grava CSV
    write_csv(municipios, OUTPUT_CSV)

    # 3. Valida integridade (apenas linhas sem campos vazios)
...
# Adapted from: https://github.com/kelvins/municipios-brasileiros
# Original author: kelvins