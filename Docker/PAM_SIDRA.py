...
def extrair_nome_variavel(excel, nome_pagina):
    ...
    # A2 = índice de linha 1, coluna 0
    celula_a2 = str(df_raw.iloc[1, 0]).strip()
    
    # Extrai o texto após "Variável - " (case-insensitive)
    match = re.search(r'[Vv]ariável\s*-\s*(.+)', celula_a2)
    if match:
        return match.group(1).strip()
    
    # Fallback: retorna o conteúdo bruto se o padrão não bater
    print(f"  Aviso: padrão 'Variável - ' não encontrado em A2 de '{nome_pagina}'. Conteúdo: '{celula_a2}'")
    return celula_a2


def converter_excel_para_csv(caminho_arquivo, caminho_saida='./data/raw/PAM_SIDRA/PAM_SIDRA.csv'):
    ...
    for nome_pagina in excel.sheet_names:
        ...
        # Extrai o nome real da variável da célula A2
        ...
        # Achata MultiIndex: Unnamed no sub-nível = coluna de ID, senão usa sub-nível (ano)
        new_cols = []
        ...
        df_longo = pd.melt(
            ...
            var_name='Ano',
            value_name=nome_variavel  # ← nome real da variável aqui
        )
    ...
    print(f"Variáveis encontradas: {[c for c in df_final.columns if c not in ['Cód.', 'Município', 'Ano']]}")
